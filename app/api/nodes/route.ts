  ...
  const nodes = db
    .prepare(
      ...
      SELECT n.id, n.name, n.type,
             (SELECT COUNT(*) FROM review r WHERE r.entity_id = n.id) AS review_count,
...
export async function PUT(request: Request) {
  ...
  const tx = db.transaction(() => {
    ...
    if (payload.id !== payload.originalId) {
      db
        .prepare('UPDATE review SET entity_id = ? WHERE entity_id = ?')
...
export async function DELETE(request: Request) {
  ...
  const row = db
    .prepare(
      `
      SELECT
        (SELECT COUNT(*) FROM review r WHERE r.entity_id = ?) AS review_count,
