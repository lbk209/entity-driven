  ...
  const edges = db
    .prepare(
      ...
      SELECT e.parent_id, e.child_id, e.relation,
             (SELECT COUNT(*) FROM review r WHERE r.entity_id = e.parent_id) AS parent_review_count,
             (SELECT COUNT(*) FROM review r WHERE r.entity_id = e.child_id) AS child_review_count
      FROM edges e
  ...
  if (!payload.force) {
    const row = db
      .prepare(
        `
        SELECT
          (SELECT COUNT(*) FROM review r WHERE r.entity_id = ?) AS parent_review_count,
          (SELECT COUNT(*) FROM review r WHERE r.entity_id = ?) AS child_review_count
      `
      )
